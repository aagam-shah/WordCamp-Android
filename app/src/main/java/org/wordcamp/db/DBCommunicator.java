...
//        contentValues.put("wcid",wc.getID());
    ...
    public static final String DB_CREATE_NOTIFICATION = "create table notification " +
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    Notif f = new Notif(type, title, descr, link, read);
                    notifList.add(f);
                } while (cursor.moveToNext());
                cursor.close();
                return notifList;
    ...
    public void addNewNotif(String type, String title, String descr, String link) {
        ContentValues contentValues = new ContentValues();
    ...
    public int getUnreadNotifs(){
        Cursor mCount= db.rawQuery("select count(*) from notification where read = 0", null);
        mCount.moveToFirst();
